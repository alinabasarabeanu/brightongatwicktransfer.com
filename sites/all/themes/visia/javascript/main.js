...
(function($) {
  ...
  $(document).ready(function() {
    ...
    $('#simple-contact-form').submit(function() {
      ...
      var action = Drupal.settings.basePath + 'simplecontact/ajax';


      $("#simple-contact-form #alert").slideUp(750, function() {
        $('#alert').hide();

        $('#simple-contact-form .form-submit')
                .after('<img src="' + Drupal.settings.basePath + '/sites/all/themes/nevia/images/ajax-loader.gif" class="loader" />')
                .attr('disabled', 'disabled');

        $.post(action, {
          name: $('#simple-contact-form #edit-name').val(),
          email: $('#simple-contact-form #edit-mail').val(),
          message: $('#simple-contact-form #edit-message').val()
        },
        function(data) {
          $('#simple-contact-form #alert').html(data);
          $('#simple-contact-form #alert').slideDown('slow');
          $('#simple-contact-form img.loader').fadeOut('slow', function() {
            $(this).remove()
          });
          $('#simple-contact-form .form-submit').removeAttr('disabled');
  ...
  (function() {
    function heroInit() {
      ...
      var heroContent = jQuery('.hero .content'),
              ...
              topMargin = (parentHeight - contentHeight) / 1.2;
...
jQuery(document).ready(function() {
  ...
  /* Prices */
  jQuery('#section3 .icon').bind('click', function() {
    if(jQuery('#section3 div.active').length) {
      jQuery('#section3 div.active').removeClass('active');
    }
    else{
